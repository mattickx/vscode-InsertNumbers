...
export class TSSprintf {
  ...
  public static sprintf(format: string, ...args: any[]): string {
    const key = format, cache = this.cache
  ...
  private static format(parse_tree: string | any[], argv: any[]) {
    let cursor = 0, tree_length = parse_tree.length, node_type = '', arg, output = [], i, k, match, pad, pad_character, pad_length, is_positive = true, sign = ''
  ...
  private static parse(fmt: string) {
    ...
    let _fmt = fmt, parse_tree = [], arg_names = 0
    while (_fmt) {
      if ((match = this.re.text.exec(_fmt)) !== null) {
        ...
        if (match[2]) {
          arg_names |= 1
          let field_match: string[] | null = []
          let field_list = [], replacement_field = match[2]
